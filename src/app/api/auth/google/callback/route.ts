...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (user) {
      // Returning user — patch any missing Google fields
      let updated = false
      ...
      isNewUser = true
      const defaultThemes = ['sejarah_indonesia', 'sains', 'pop_culture']

      if (guestUid) {
        const guestUser = await User.findOne({ uniqueUserId: guestUid })
        if (guestUser) {
          // Guest record in DB — link Google credentials to it
          ...
          // Guest played locally only — create DB record with their guest ID
          ...
          await ThemeScore.insertMany(
            defaultThemes.map((theme) => ({ userId: user!._id, theme, points: 0, seenCardIds: [] }))
          )
        }
      } else {
        // Fresh Google sign-up
